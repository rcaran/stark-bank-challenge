...
def check_event_bus() -> dict[str, any]:
    ...
    try:
        ...
        return {"status": "error", "message": "EventBus not properly initialized"}
    except Exception as e:
        logger.error(f"EventBus health check failed: {e!s}")
        return {"status": "error", "message": f"EventBus error: {e!s}"}
...
def check_health() -> dict[str, any]:
    ...
    checks = {"database": db_check["status"], "event_bus": event_bus_check["status"]}

    # If any check failed, overall status is unhealthy
    overall_status = (
        "healthy" if all(status == "ok" for status in checks.values()) else "unhealthy"
    )
    ...
    if overall_status == "unhealthy":
        response["details"] = {"database": db_check, "event_bus": event_bus_check}
