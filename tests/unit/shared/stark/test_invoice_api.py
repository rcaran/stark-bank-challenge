...
from src.shared.stark.invoice_api import StarkInvoiceAPI, InvoiceResponse
...
def test_create_invoice_validation_error(api, mocker):
    ...
    with pytest.raises(ValidationError):
        # Passing wrong type to fail internally before stark call if I had validation there?
        # Or mock stark raising InputErrors.
        # But wait, my code checks 'isinstance(amount, int)'.
        api.create_invoice(amount="1000", tax_id="123", name="John", due_date=date.today())
    
    assert mock_sleep.call_count == 0  # Should NOT retry


def test_get_invoice_success(api, mock_stark_invoice_get):
    mock_invoice = MagicMock(id="inv-123", amount=1000, due=date.today(), status="paid", fine=0, interest=0, tags=[], descriptions=[])
...
def test_list_invoices_success(api, mock_stark_invoice_query):
    mock_invoice = MagicMock(id="inv-123", amount=1000, due=date.today(), status="created", fine=0, interest=0, tags=[], descriptions=[])
