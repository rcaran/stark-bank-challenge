...
def test_transaction_management():
    # Only test if context manager works, actual rollback logic tested in integration usually
    db = DatabaseConnection()
    
    with db.get_db() as conn:
        conn.execute("CREATE TABLE IF NOT EXISTS test_transaction (id INTEGER PRIMARY KEY)")
