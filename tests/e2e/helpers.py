...
def create_test_invoice(
    client: TestClient, invoice_data: dict, api_key: str = "test-api-key"
) -> dict:
    """
    ...
    response = client.post(
        "/invoices", json=invoice_data, headers={"X-API-Key": api_key}
    )
...
def assert_invoice_exists(
    db_connection, invoice_id: str, expected_status: InvoiceStatus | None = None
) -> InvoiceModel:
    """
...
def assert_transfer_created(
    db_connection,
    invoice_id: str,
    expected_status: TransferStatus | None = None,
...
def assert_transfer_not_exists(db_connection, invoice_id: str) -> None:
    """
...
def assert_invoice_paid(
    db_connection, invoice_id: str, expected_net_amount: float | None = None
) -> InvoiceModel:
    """
    ...
    assert invoice.paid_at is not None, (
        f"Invoice {invoice_id} missing paid_at timestamp"
    )
...
def assert_transfer_completed(db_connection, transfer_id: str) -> TransferModel:
    """
...
def assert_transfer_failed(
    db_connection, transfer_id: str, expect_error_message: bool = True
) -> TransferModel:
    """
...
def get_invoice_by_stark_id(
    db_connection, stark_invoice_id: str
) -> InvoiceModel | None:
    """
...
def get_transfer_by_stark_id(
    db_connection, stark_transfer_id: str
) -> TransferModel | None:
    """
...
def count_invoices_by_status(db_connection, status: InvoiceStatus) -> int:
    """
...
def count_transfers_by_status(db_connection, status: TransferStatus) -> int:
    """
