...
class TestErrorScenarios:
    ...
    def test_invalid_webhook_signature(
        ...
        assert (
            "signature" in error_msg.lower() or "unauthorized" in error_msg.lower()
        ...
        assert invoice.net_amount is None, (
            "Net amount should not be set on rejected webhook"
        )
        ...
        response = simulate_webhook(
            client=e2e_app, webhook_type="invoice", payload=webhook_invoice_payload
        )
        ...
        webhook_transfer_payload["event"]["log"]["transfer"]["id"] = (
            transfer.stark_transfer_id
        )
        webhook_transfer_payload["event"]["log"]["transfer"]["externalId"] = (
        ...
        assert response.status_code == 401, (
            f"Expected 401 for transfer webhook, got {response.status_code}"
        )
    ...
    def test_stark_api_timeout(self, e2e_app, e2e_db, api_key_header, mock_stark_api):
        ...
        response = e2e_app.post("/invoices", json=invoice_data, headers=api_key_header)
        ...
        invoices = invoice_repo.list(limit=100, offset=0)
    ...
    def test_database_error_recovery(
        self, e2e_app, e2e_db, api_key_header, sample_webhook_invoice_paid
    ):
        """
        ...
        with patch.object(
            InvoiceRepository, "update", side_effect=Exception("Database locked")
        ):
            # Step 3: Send webhook - should fail internally but return 200
            # --------------------------------------------------------------
            response = simulate_webhook_raw(
                client=e2e_app, webhook_type="invoice", payload=webhook_payload
            )
            ...
            has_error = (
                ...
                or response_json.get("status") != "ok"
                or "processing_error" in str(response_json).lower()
            )
        ...
        assert invoice.fee is None, "Fee should not be set after DB error"
        assert invoice.net_amount is None, "Net amount should not be set after DB error"
    ...
    def test_webhook_with_unknown_invoice(
        self, e2e_app, e2e_db, sample_webhook_invoice_paid
    ):
        """
        ...
        assert isinstance(response_json, dict), "Response should be a valid JSON object"
