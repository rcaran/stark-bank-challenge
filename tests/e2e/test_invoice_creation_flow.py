...
class TestInvoiceCreationFlow:
    ...
    def test_invoice_creation_success(
        ...
        for db_invoice in db_invoices:
            ...
            logger.debug(f"  - Invoice {db_invoice['id']}: {db_invoice['customer_name']} - {db_invoice['amount']/100:.2f}")
        ...
        invoices_from_db = repository.list(status=InvoiceStatus.CREATED, limit=100, offset=0)
    ...
    def test_invoice_creation_handles_partial_failures(
        ...
        cursor = e2e_db.connection.cursor()
        cursor.execute("SELECT COUNT(*) as count FROM invoices WHERE status = ?", 
                      (InvoiceStatus.CREATED.value,))
        success_count = cursor.fetchone()["count"]
        
        assert success_count == 2, f"Expected 2 successful invoices, got {success_count}"
        
        logger.info(f"✓ Partial failure handled: 2 success, 1 failed")
